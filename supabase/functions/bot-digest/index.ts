// bot-digest v1 - pull-only service digest for the Telegram bot /digest command.
//
// AUTH: shared secret in header `x-bot-secret` must match BOT_SHARED_SECRET env.
// BODY: { telegram_id: string|number, kind?: "day"|"week", send?: boolean }
...
Deno.serve(async (req: Request) => {
  ...
  const kind = String(body.kind ?? "day").toLowerCase() === "week" ? "week" : "day";
  ...
  if (status === "not_linked") {
    ...
    text = kind === "week" ? copy.empty_week : copy.empty_day;
  } else if (status === "ok") {
